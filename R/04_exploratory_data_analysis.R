...
## The functions `aggregate` and `apply` --------------------------------------

table(iris$Species)

# sepal length mean per species
tapply(X = iris$Sepal.Length, INDEX = list(iris$Species), FUN = mean) #tapply es para hacer una función especifica en los datos
...
hist(iris$Petal.Length)
par(mfrow = c(1, 1)) #Toca ponerlo porque o sino los siguientes siguen saliendo en un 2x2
