...
ggplot(data = cat) +
  ...
  geom_abline(aes(slope = 1,  intercept = 0)) + #SI el predict fuera igual al real sería igual a esta linea
  theme_classic()


## Model diagnostics -----------------------------------------------------------
par(mfrow = c(2, 2))
plot(mod_cat) #Los numeros son el orden de la fila en el dataframe
par(mfrow = c(1, 1))
